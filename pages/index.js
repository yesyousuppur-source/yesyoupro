          ...
          {tab==="gst"&&(
            <div className="fbox fa">
              ...
              {(()=>{
                ...
                return(<>
                  <div className="prow">
                    <div className="pfield"><label>Selling Price (₹)</label><input type="number" className="di" placeholder="500" value={sell} onChange={e=>setSell(e.target.value)}/></div>
                    <div className="pfield"><label>GST Rate</label>
                      <select className="di" value={rate} onChange={e=>setRate(e.target.value)}>
                  ...
                  <button className="cbtn" style={{width:"100%"}} onClick={()=>{const s=parseFloat(sell)||0;const r=parseFloat(rate)||0;const base=s/(1+r/100);const gst=s-base;setRes({sell:s.toFixed(2),base:base.toFixed(2),gst:gst.toFixed(2),cgst:(gst/2).toFixed(2),sgst:(gst/2).toFixed(2),rate:r});}}>Calculate GST</button>
                  {res&&<div style={{marginTop:12}} className="fa">
                    <div style={{background:"rgba(99,102,241,.08)",border:"1px solid rgba(99,102,241,.2)",borderRadius:12,padding:14,textAlign:"center",marginBottom:10}}>
                      ...
                      <div style={{fontSize:28,fontWeight:900,color:"#ef4444"}}>₹{res.gst}</div>
                    </div>
                    <div className="presult">
                      {[{l:"Selling Price",v:"₹"+res.sell,c:"#f8fafc"},{l:"Base Price",v:"₹"+res.base,c:"#10b981"},{l:"CGST",v:"₹"+res.cgst,c:"#f59e0b"},{l:"SGST",v:"₹"+res.sgst,c:"#f59e0b"}].map(r=>(
          ...
          {tab==="shipping"&&(
            <div className="fbox fa">
              ...
              {(()=>{
                ...
                const [cod,setCod]=React.useState("no");
                const [res,setRes]=React.useState(null);
                return(<>
                  <div className="prow">
                    <div className="pfield"><label>Weight (kg)</label><input type="number" className="di" placeholder="0.5" step="0.1" value={w} onChange={e=>setW(e.target.value)}/></div>
                    <div className="pfield"><label>Zone</label>
                      <select className="di" value={zone} onChange={e=>setZone(e.target.value)}>
                    ...
                    <div className="pfield"><label>Payment</label>
                      <select className="di" value={cod} onChange={e=>setCod(e.target.value)}>
                  ...
                  <button className="cbtn" style={{width:"100%"}} onClick={()=>{
                    const wt=parseFloat(w)||0.5;const isCod=cod==="yes";
                    const zm={local:1,zone1:1.3,zone2:1.6,zone3:2}[zone]||1;
                    const couriers=[{n:"Delhivery",b:35,p:45,c:"#ef4444"},{n:"Shiprocket",b:40,p:50,c:"#6366f1"},{n:"Amazon FBA",b:30,p:40,c:"#f59e0b"},{n:"Ekart",b:28,p:38,c:"#10b981"},{n:"Shadowfax",b:32,p:42,c:"#8b5cf6"},{n:"XpressBees",b:36,p:46,c:"#f97316"}]
                      .map(c=>({...c,cost:Math.round((c.b+c.p*Math.max(0.5,wt))*zm+(isCod?35:0))}))
                      .sort((a,b)=>a.cost-b.cost);
                    setRes({couriers,isCod});
                  }}>Compare Rates</button>
                  {res&&<div style={{marginTop:12}} className="fa">
                    {res.couriers.map((c,i)=>(
                    ...
                    {res.isCod&&<div style={{fontSize:11,color:"#f59e0b",padding:"6px 0"}}>COD charge (+₹35) included</div>}
                  </div>}
                </>);
              })()}
            </div>
